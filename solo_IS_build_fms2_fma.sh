#/bin/bash
mkdir -p build/IS_only_fms2_fma/
(cd build/IS_only_fms2_fma/; rm -f path_names; \
...
source ./build/env_c5
make -C ./build/IS_only_fms2_fma NETCDF=3 REPRO=1 ISA='-fma -qno-opt-dynamic-align' -j 8 MOM6
