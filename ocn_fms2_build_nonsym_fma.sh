#/bin/bash
mkdir -p build/ocean_only_fms2_nonsym_fma/
(cd build/ocean_only_fms2_nonsym_fma/; rm -f path_names; \
...
source ./build/env_c5
make -C ./build/ocean_only_fms2_nonsym_fma NETCDF=3 REPRO=1 ISA='-fma -qno-opt-dynamic-align' -j 8 MOM6
